...
<div class="max-w-5xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-lg">
    ...
    @if ($ticketDetails)
        <table class="table-auto w-full border border-gray-300 mb-4">
            ...
            <tbody>
                <tr>
                    <td class="py-2 px-4 border-b">{{ $ticketDetails['name'] }}</td>
                    <td class="py-2 px-4 border-b">{{ $ticketDetails['phone_number'] }}</td>
                    <td class="py-2 px-4 border-b">{{ $ticketDetails['email'] }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="table-auto w-full border border-gray-300 mb-4">
            ...
            <tbody>
                @foreach ($ticketDetails['tickets'] as $ticketTypeId => $details)
                    <tr>
                        ...
                        <td class="text-right py-2 px-4 border-b">KES {{ number_format($details['subtotal'], 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>

        <h3 class="text-lg font-bold text-right mb-4">Total Amount: KES {{ number_format($ticketDetails['grandTotal'], 2) }}</h3>
    @else
    ...
    <form action="{{ secure_url('payments/initiateStkPush') }}" method="GET">
        ...
        <input type="hidden" name="grand_total" value="{{ $ticketDetails['grandTotal'] }}">
        ...
        <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Proceed</button>
    </form>
</div>
