...
<x-app-layout title="Permissions">
    ...
    <div class="max-w-4xl max-h-3xl mx-auto mt-2 px-4">
                ...
                <form action="{{ url('permissions/' . $permission->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Permission
                            Name</label>
                        <input id="name" type="text" value="{{ $permission->name }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            name="name" required>
                    </div>
                    <button type="submit"
                        class="bg-red-500 hover:bg-red-700 mb-4 text-white font-bold py-2 px-4  rounded float-right">Update
                        Permission</button>
                </form>
            </div>
        </div>
